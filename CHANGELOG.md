...
### 🎯 核心功能优化
- **群昵称智能判断机制**
  - 实现智能三层判断，防止错误覆盖用户手动修改的正确群昵称
  - 情况A：当前群昵称已正确 → 仅更新数据库，保持群昵称不变
  - 情况B：API缓存未刷新 → 采用保守策略，跳过修改
  - 情况C：API返回新数据 → 正常更新群昵称和数据库

### 🔧 技术改进
- **新增工具函数**
  - 添加 `extractBuidUsernameFromNickname()` 函数 (src/utils/helpers.ts)
  - 用于从标准格式群昵称中提取B站用户名
  - 支持格式：`B站名称（ID:MC名称）`

- **智能验证逻辑升级**
  - 在群昵称设置前先解析当前昵称
  - 对比 API返回/数据库缓存/当前群昵称 三方数据
  - 根据对比结果智能决策：更新/跳过/仅更新数据库

- **4处代码位置同步优化**
  - src/utils/message-utils.ts: 主逻辑分支 + 异常分支
  - src/index.ts: 主逻辑分支 + 异常分支
  - 确保所有场景下逻辑一致

### 🐛 Bug修复
- **修复用户手动改名被覆盖的问题**
  - 场景：用户在B站改名 → 手动修改群昵称 → 系统误将昵称改回旧的
  - 原因：未判断当前群昵称是否已包含正确的B站用户名
  - 解决：新增提取+对比逻辑，识别用户手动修改的正确昵称

- **防止API缓存误导**
  - 当API返回与数据库一致的旧数据时，不再盲目修改群昵称
  - 采用保守策略，避免在API缓存未刷新时造成错误修改

### 📊 改进效果
| 场景 | 优化前 | 优化后 |
|------|--------|--------|
| 用户手动改正确昵称 | ❌ 被改回旧的 | ✅ 保持用户的，更新数据库 |
| API缓存未刷新 | ⚠️ 可能误改 | ✅ 保守不改 |
| API返回新数据 | ✅ 正常更新 | ✅ 正常更新 |

### 📝 详细日志输出
- 新增调试日志：显示从群昵称中提取的B站用户名
- 新增判断日志：显示智能判断的决策过程和原因
- 优化警告日志：在保守策略触发时给出明确提示
