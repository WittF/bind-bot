...
export class WhitelistHandler extends BaseHandler {
  ...
  private registerAddCommand(parent: any): void {
    parent.subcommand('.add <serverIdOrName:string> [...targets:string]', '申请/添加服务器白名单')
      .action(async ({ session }: { session: Session }, serverIdOrName: string, ...targets: string[]) => {
        try {
          const normalizedUserId = this.deps.normalizeQQId(session.userId)
          ...
          if (!serverIdOrName) {
            ...
            return this.deps.sendMessage(session, [h.text('请提供服务器名称或ID\n使用 mcid whitelist servers 查看可用服务器列表')])
          }
          ...
          if (!server) {
            ...
            return this.deps.sendMessage(session, [h.text(`未找到名称或ID为"${serverIdOrName}"的服务器\n使用 mcid whitelist servers 查看可用服务器列表`)])
          }
          ...
          if (targets && targets.length > 0) {
            ...
            if (!await this.isAdmin(session.userId)) {
              ...
              return this.deps.sendMessage(session, [h.text('只有管理员才能为其他用户添加白名单')])
            }
            ...
            if (targets.length === 1) {
              ...
              if (usersWithTag.length > 0) {
                ...
                await this.deps.sendMessage(session, [h.text(`找到${targets.length}个有标签"${tagName}"的已绑定用户，开始添加白名单...`)])
              }
              // 如果没有找到标签，将继续按单个用户处理
            }
            ...
            if (targets.length === 1) {
              ...
              if (this.isInServerWhitelist(targetBind, server.id)) {
                ...
                return this.deps.sendMessage(session, [h.text(`用户 ${normalizedTargetId} 已在服务器"${server.name}"的白名单中`)])
              }
              ...
              if (result) {
                ...
                return this.deps.sendMessage(session, [h.text(`已成功为用户 ${normalizedTargetId} 添加服务器"${server.name}"的白名单`)])
              } else {
                this.logger.error('白名单', `管理员QQ(${normalizedUserId})为QQ(${normalizedTargetId})添加服务器"${server.name}"白名单失败`)
                return this.deps.sendMessage(session, [h.text(`为用户 ${normalizedTargetId} 添加服务器"${server.name}"白名单失败，请检查RCON连接和命令配置`)])
              }
            }
            ...
            // 发送开始处理的通知
            await this.deps.sendMessage(session, [h.text(`开始为${targets.length}个用户添加服务器"${server.name}"的白名单，请稍候...`)])
            ...
            for (let i = 0; i < targets.length; i++) {
              ...
              try {
                ...
                if (targets.length > 5 && (i + 1) % 5 === 0) {
                  ...
                  await this.deps.sendMessage(session, [h.text(`批量添加白名单进度: ${progress}% (${i + 1}/${targets.length})\n成功: ${successCount} | 失败: ${failCount} | 跳过: ${skipCount}`)])
                }
              } catch (error) {
                failCount++
          ...
          if (!server.allowSelfApply && !await this.isAdmin(session.userId)) {
            ...
            return this.deps.sendMessage(session, [h.text(`服务器"${server.name}"不允许自助申请白名单，请联系管理员`)])
          }
          ...
          if (this.isInServerWhitelist(selfBind, server.id)) {
            ...
            return this.deps.sendMessage(session, [h.text(`您已在服务器"${server.name}"的白名单中`)])
          }
          ...
          if (result) {
            ...
            return this.deps.sendMessage(session, [h.text(`已成功添加服务器"${server.name}"的白名单`)])
          } else {
            this.logger.error('白名单', `QQ(${normalizedUserId})添加服务器"${server.name}"白名单失败`)
            return this.deps.sendMessage(session, [h.text(`添加服务器"${server.name}"白名单失败，请联系管理员`)])
          }
        } catch (error) {
          const normalizedUserId = this.deps.normalizeQQId(session.userId)
      ...
      .action(async ({ session }: { session: Session }, serverIdOrName: string, ...targets: string[]) => {
        try {
          ...
          if (targets && targets.length > 0) {
            ...
            if (targets.length === 1) {
              ...
              if (usersWithTag.length > 0) {
                ...
                await this.deps.sendMessage(session, [h.text(`找到${targets.length}个有标签"${tagName}"的已绑定用户，开始移除白名单...`)])
              }
              // 如果没有找到标签，将继续按单个用户处理
            }
            ...
            if (targets.length === 1) {
              ...
              if (!this.isInServerWhitelist(targetBind, server.id)) {
                ...
                return this.deps.sendMessage(session, [h.text(`用户 ${normalizedTargetId} 不在服务器"${server.name}"的白名单中`)])
              }
              ...
              if (result) {
                ...
                return this.deps.sendMessage(session, [h.text(`已成功为用户 ${normalizedTargetId} 移除服务器"${server.name}"的白名单`)])
              } else {
                this.logger.error('白名单', `管理员QQ(${normalizedUserId})为QQ(${normalizedTargetId})移除服务器"${server.name}"白名单失败`)
                return this.deps.sendMessage(session, [h.text(`为用户 ${normalizedTargetId} 移除服务器"${server.name}"白名单失败，请检查RCON连接和命令配置`)])
              }
            }
            ...
            // 发送开始处理的通知
            await this.deps.sendMessage(session, [h.text(`开始为${targets.length}个用户移除服务器"${server.name}"的白名单，请稍候...`)])
            ...
            for (let i = 0; i < targets.length; i++) {
              ...
              try {
                ...
                if (targets.length > 5 && (i + 1) % 5 === 0) {
                  const progress = Math.round(((i + 1) / targets.length) * 100)
                  await this.deps.sendMessage(session, [h.text(`批量移除白名单进度: ${progress}% (${i + 1}/${targets.length})\n成功: ${successCount} | 失败: ${failCount} | 跳过: ${skipCount}`)])
                }
              } catch (error) {
                failCount++
                results.push(`❌ ${normalizedTargetId}: 处理出错`)
          ...
          if (!this.isInServerWhitelist(selfBind, server.id)) {
            ...
            return this.deps.sendMessage(session, [h.text(`您不在服务器"${server.name}"的白名单中`)])
          }
          ...
          if (result) {
            ...
            return this.deps.sendMessage(session, [h.text(`已成功移除服务器"${server.name}"的白名单`)])
          } else {
            this.logger.error('白名单', `管理员QQ(${normalizedUserId})移除服务器"${server.name}"白名单失败`)
            return this.deps.sendMessage(session, [h.text(`移除服务器"${server.name}"白名单失败，请检查RCON连接和命令配置`)])
          }
        } catch (error) {
          const normalizedUserId = this.deps.normalizeQQId(session.userId)
      ...
      .action(async ({ session }: { session: Session }, serverIdOrName: string) => {
        try {
          ...
          if (!serverIdOrName) {
            ...
            return this.deps.sendMessage(session, [h.text('请提供服务器ID或名称\n使用 mcid whitelist servers 查看可用服务器列表')])
          }
      ...
      .action(async ({ session }: { session: Session }, serverIdOrName: string) => {
        try {
          ...
          if (server.enabled === false) {
            ...
            return this.deps.sendMessage(session, [h.text(`服务器"${server.name}"已停用，无法添加白名单`)])
          }

          // 发送开始执行的通知
          await this.deps.sendMessage(session, [h.text(`开始批量添加白名单到服务器"${server.name}"，请稍候...`)])
          ...
          for (let i = 0; i < validBinds.length; i++) {
            ...
            try {
              ...
              if (this.isInServerWhitelist(bind, server.id)) {
                ...
                if (currentFailRate > 0.5 && (successCount + failCount) >= 5) {
                  ...
                  await this.deps.sendMessage(session, [h.text(`⚠️ 批量添加白名单操作已中止: 失败率过高(${Math.round(currentFailRate * 100)}%)，请检查服务器连接`)])
                  break
                }
              ...
              if (error.message.includes('失败率过高')) {
                await this.deps.sendMessage(session, [h.text(`⚠️ 批量添加白名单操作已中止: ${error.message}`)])
                break
              }
            }
            ...
            if (i === 0 || progress - lastNotifiedProgress >= 20 || i === validBinds.length - 1) {
              await this.deps.sendMessage(session, [h.text(`批量添加白名单进度: ${progress}%，已处理${processedCount}/${validBinds.length}个用户\n成功: ${successCount} | 失败: ${failCount} | 跳过: ${skipCount}`)])
          ...
          return this.deps.sendMessage(session, [h.text(`批量添加服务器"${server.name}"白名单完成\n共处理${validBinds.length}个有效用户\n✅ 成功: ${successCount} 个\n❌ 失败: ${failCount} 个\n⏭️ 跳过(已在白名单): ${skipCount} 个\n\n如需查看详细日志，请查看服务器日志文件`)])
        } catch (error) {
          const normalizedUserId = this.deps.normalizeQQId(session.userId)
