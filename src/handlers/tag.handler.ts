...
export class TagHandler extends BaseHandler {
  ...
  private async handleTagAdd(session: Session, tagName: string, ...targets: string[]): Promise<any> {
    try {
      ...
      if (!this.validateTagName(tagName)) {
        ...
        return this.deps.sendMessage(session, [h.text('标签名称只能包含中文、字母、数字、下划线和连字符')])
      }
      ...
      if (targets.length === 1) {
        ...
        if (targetBind.tags && targetBind.tags.includes(tagName)) {
          ...
          return this.deps.sendMessage(session, [h.text(`用户 ${normalizedTargetId} 已有标签"${tagName}"`)])
        }
        ...
        return this.deps.sendMessage(session, [h.text(`已成功为用户 ${normalizedTargetId} 添加标签"${tagName}"`)])
      }
      this.logger.info('标签', `管理员QQ(${normalizedUserId})尝试批量为${targets.length}个用户添加标签"${tagName}"`, true)
      await this.deps.sendMessage(session, [h.text(`开始为${targets.length}个用户添加标签"${tagName}"，请稍候...`)])
  ...
  private async handleTagRemove(session: Session, tagName: string, ...targets: string[]): Promise<any> {
    try {
      const normalizedUserId = this.deps.normalizeQQId(session.userId)
      if (!await this.isAdmin(session.userId)) {
        this.logger.warn('标签', `权限不足: QQ(${normalizedUserId})不是管理员`)
      ...
      if (targets.length === 1) {
        ...
        if (!targetBind.tags || !targetBind.tags.includes(tagName)) {
          ...
          return this.deps.sendMessage(session, [h.text(`用户 ${normalizedTargetId} 没有标签"${tagName}"`)])
        }
        ...
        return this.deps.sendMessage(session, [h.text(`已成功为用户 ${normalizedTargetId} 移除标签"${tagName}"`)])
      }
      this.logger.info('标签', `管理员QQ(${normalizedUserId})尝试批量为${targets.length}个用户移除标签"${tagName}"`, true)
      await this.deps.sendMessage(session, [h.text(`开始为${targets.length}个用户移除标签"${tagName}"，请稍候...`)])
      let successCount = 0, failCount = 0, skipCount = 0
      const results: string[] = []
      for (let i = 0; i < targets.length; i++) {
        const target = targets[i]
  ...
  private async handleTagList(session: Session, target?: string): Promise<any> {
    try {
      ...
      if (target) {
        ...
        return this.deps.sendMessage(session, [h.text(`用户 ${normalizedTargetId} 的标签:\n${tagList}`)])
      }
  ...
  private async handleTagFind(session: Session, tagName: string): Promise<any> {
    try {
      ...
      return this.deps.sendMessage(session, [h.text(`有标签"${tagName}"的用户 (共${usersWithTag.length}人):\n${userList}`)])
    } catch (error) {
      const normalizedUserId = this.deps.normalizeQQId(session.userId)
  ...
  private async handleTagRename(session: Session, oldTagName: string, newTagName: string): Promise<any> {
    try {
      const normalizedUserId = this.deps.normalizeQQId(session.userId)
      if (!await this.isAdmin(session.userId)) {
        this.logger.warn('标签', `权限不足: QQ(${normalizedUserId})不是管理员`)
      ...
      if (!this.validateTagName(newTagName)) {
        ...
        return this.deps.sendMessage(session, [h.text('新标签名称只能包含中文、字母、数字、下划线和连字符')])
      }
      const allBinds = await this.repos.mcidbind.findAll()
      ...
      if (usersWithNewTag.length > 0) {
        ...
        return this.deps.sendMessage(session, [h.text(`新标签"${newTagName}"已存在，请选择其他名称`)])
      }
      this.logger.info('标签', `管理员QQ(${normalizedUserId})开始将标签"${oldTagName}"重命名为"${newTagName}"`, true)
      await this.deps.sendMessage(session, [h.text(`找到${usersWithOldTag.length}个用户有标签"${oldTagName}"，开始重命名为"${newTagName}"...`)])
  ...
  private async handleTagDeleteAll(session: Session, tagName: string): Promise<any> {
    try {
      ...
      await this.deps.sendMessage(session, [h.text(`找到${usersWithTag.length}个用户有标签"${tagName}"，开始批量删除...`)])
      let successCount = 0, failCount = 0
