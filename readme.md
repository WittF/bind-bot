...
### v1.4.3 (最新)
- 优化批量处理流程：提升大规模白名单操作的稳定性
- 改进错误处理：细化错误提示，提供更详细的错误原因
- 修复潜在的标签操作Bug：增强标签管理的可靠性
- 完善使用说明文档：更新命令格式和配置项描述
- 提升代码可维护性：改进内部代码架构，优化资源占用

### v1.4.1
- 新增标签管理功能：管理员可以为用户添加标签，用于批量管理
- 批量白名单操作支持：`mcid whitelist add/remove`命令现在支持批量@用户
- 标签批量操作：支持通过标签名称批量添加/移除白名单
- 优化单个用户操作反馈：保留简洁的成功/失败消息，仅批量操作显示详细统计
- 数据库结构升级：添加tags字段存储用户标签
- 改进错误处理：优化了标签识别逻辑，避免将QQ号误识别为标签
